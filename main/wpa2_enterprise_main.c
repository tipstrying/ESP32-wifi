...
static const char *TAG = "wifiLearn";
...
static void initialise_wifi(void)
{
    ...
    if (sscanf((char *)CONFIG_STATIC_IP, "%d.%d.%d.%d", addr, addr + 1, addr + 2, addr + 3) == 4)
    {
        IP4_ADDR(&ip_info.ip, addr[0], addr[1], addr[2], addr[3]);
    }
    else
    {
        ESP_LOGE(TAG, "ip addr set error: %s", CONFIG_STATIC_IP);
    }
    if (sscanf((char *)CONFIG_STATIC_GATEWAY, "%d.%d.%d.%d", addr, addr + 1, addr + 2, addr + 3) == 4)
    {
        IP4_ADDR(&ip_info.gw, addr[0], addr[1], addr[2], addr[3]);
    }
    else
    {
        ESP_LOGE(TAG, "gateway addr set error: %s", CONFIG_STATIC_GATEWAY);
    }
    if (sscanf((char *)CONFIG_STATIC_NETMASK, "%d.%d.%d.%d", addr, addr + 1, addr + 2, addr + 3) == 4)
    {
        IP4_ADDR(&ip_info.netmask, addr[0], addr[1], addr[2], addr[3]);
    }
    else
    {
        ESP_LOGE(TAG, "netmask addr set error: %s", CONFIG_STATIC_NETMASK);
    }
